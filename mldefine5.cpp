...
uvector MLdefine5::phi(const state &s,const action &a){
	uvector v=zero();
	Cards newHand = diffHand(s.myCards(),a);
	std::vector<Hand> hs = getAIHand(newHand);
	for(int i=0;i<hs.size();i++){
		int a=11*s.info.rank[s.info.mypos]+max(0,min(10,bitCount(s.myCards())-1));
		v[512*a+getIndex(hs[i],s.info.rev)]=1.0;
	}
	return v;
}

Hand MLdefine5::optimizedAI(const fieldInfo & info,Cards myCards,Cards oppCards
			,double *theta){
	...
	int a=11*info.rank[info.mypos]+max(0,min(10,tehuda-1));
...
void MLdefine5::show(const uvector &theta){
	for(int q=0;q<14;q++){
		std::cout << q << " : ";
		for(int k=0;k<4;k++){
...
uvector MLdefine5::merge(uvector notrev,uvector rev){
